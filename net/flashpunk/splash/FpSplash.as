package net.flashpunk.splash 
{
	 ...
	 */
	public class FpSplash extends Entity
	{
		/**
		 * Embedded graphics.
		 */
		...
		public var leftImage:Image = new Image(SPLASH_LEFT);
		public var rightImage:Image = new Image(SPLASH_RIGHT);
		 ...
		 */
		public function FpSplash(color:uint = 0xFF3366, bgColor:uint = 0x202020, fadeTime:Number = .5, spinTime:Number = 2, spinPause:Number = .5, spins:Number = 720) 
		{
			...
			x = FP.width / 2;
			y = FP.height / 2;
			graphic = new Graphiclist(leftImage, rightImage, cog, lines, fade);
			...
			leftImage.smooth = true;
			leftImage.originX = leftImage.width;
			leftImage.originY = leftImage.height / 2;
			leftImage.x -= leftImage.originX + cog.width / 4 + 4;
			leftImage.y -= leftImage.originY;
			leftX = leftImage.x;
			
			// Set the right text properties.
			rightImage.color = color;
			rightImage.smooth = true;
			rightImage.originY = rightImage.height / 2;
			rightImage.x += cog.width / 4;
			rightImage.y -= rightImage.originY;
			rightX = rightImage.x;
		...
		override public function update():void 
		{
			...
			leftImage.x = leftX - t * FP.width / 2;
			rightImage.x = rightX + t * FP.width / 2;
			leftImage.scaleY = rightImage.scaleY = tween.scale;
			leftImage.alpha = rightImage.alpha = Ease.cubeIn(tween.scale);
